...
class UFSRNetModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.netG = UFSRNet(res_depth=opt.res_depth, norm_type=opt.Gnorm, att_name=opt.att_name, bottleneck_size=opt.bottleneck_size)
        ...
        if self.isTrain:
            ...
            # self.Textures = loss.loss_Textures(self.img_HR, self.img_SR) ###
    ...
    def backward_G(self):
        ...
        # Textures loss
        # self.loss_Textures = loss.loss_Textures(self.img_SR, self.img_HR) ###
        # self.loss_Textures.backward(retain_graph=True)
        # total loss
        # self.total_loss = self.loss_Pix * 0.9 + self.loss_Textures * 0.1
